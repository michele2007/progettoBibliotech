...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($utente) {
        ...
        $passwordCorretta =
            (!empty($utente['passw_hash']) && password_verify($password, $utente['passw_hash']))
            || $password === $utente['passw'];
        ...
        db()->prepare("
            UPDATE utente SET tentativi_login = tentativi_login + 1
            WHERE id_utente = ?
        ")->execute([$utente['id_utente']]);
        ...
        if ($utente['tentativi_login'] >= 3) {
            ...
            db()->prepare("
                UPDATE utente
                SET reset_token=?, reset_scadenza=DATE_ADD(NOW(), INTERVAL 30 MINUTE)
                WHERE id_utente=?
